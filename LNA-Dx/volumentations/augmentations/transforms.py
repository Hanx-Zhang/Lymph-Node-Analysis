...
class GaussianNoise(Transform):  # 默认均值是0，对于已经标准化的图片合适此高斯噪声，而归一化的图片可能不太适合
...
class RandomRotate90(DualTransform):
    def __init__(self, axes=(0,1), always_apply=False, p=0.5):  # P可以变为1,因为 有 0 度旋转
        super().__init__(always_apply, p)
        self.axes = axes # The array is rotated in the plane defined by the axes.  Axes must be different.

    def apply(self, img, factor): # factor: Number of times the array is rotated by 90 degrees.
        return np.rot90(img, factor, axes=self.axes)

    def get_params(self, **data):
        return {"factor": random.randint(0, 3)}  # 这个factor 是为了随机转 0 90 180 270 度用的
