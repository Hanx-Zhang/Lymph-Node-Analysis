...
def dx_root() -> Path:
    """Return the LNA-Dx package root that contains main.py and test.py."""

    return Path(__file__).resolve().parents[1]


def default_external_data_root() -> Path:
    """Return the original workspace-level Data directory used by LNA-Dx_v1.

    LNA-Dx_v1 lived directly under the workspace and used ../../Data.
    The GitHub-ready copy lives at submit/Lymph-Node-Analysis/LNA-Dx, so the
    equivalent relative location is four directories above LNA-Dx.
    """

    return dx_root().parents[3] / "Data"
